...
public class VerifyUser extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            codeDB = userDB.getCode(verifyUser.getEmail());
            if (userCode.equals(String.valueOf(codeDB))) {
                userDB.verifyUser(verifyUser.getEmail());
                verifyUser.setStatus("Active");
                session.setAttribute("user", verifyUser);
                response.sendRedirect("LoadMainPage");
                return;
            } else {
                session.setAttribute("notification", true);
                session.setAttribute("failed", "Invalid code, try again");
